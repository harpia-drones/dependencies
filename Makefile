...
ifeq ($(lang),python)
	...
	@cp /root/dependencies/package_creation/templates/minimum_node.py $(name)_bringup/minimum_node.py
else ifeq ($(lang),cpp)
...
# remove the unused directories
	...
	@cd $(name)_description && mkdir urdf launch meshes config
	@cd $(name)_description/config && mkdir rviz gazebo
